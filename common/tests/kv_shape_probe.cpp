...
void nestingGoesThreeDeep(probe::Context& ctx)
{
    ...
    ctx.check(config.getU64("daemon.workers", 0) == 4, "one dedent closes the inner section only");
...
void carriageReturnsAreNotPartOfAValue(probe::Context& ctx)
{
    ...
    ctx.check(config.getU64("workers", 0) == 4, "a number is still a number");
...
void aRepeatedKeyTakesItsLastValue(probe::Context& ctx)
{
    ...
    ctx.check(config.getU64("workers", 0) == 9, "the last value wins at the top level");
