...
inline constexpr auto AcquireTimeout = std::chrono::seconds{1};

// Poll-thread cadence. LocalPeerState seeds its pollInterval from this.
inline constexpr auto DefaultPollInterval = std::chrono::microseconds{10};
...
inline constexpr auto LivenessProbeInterval = std::chrono::microseconds{500};
...
inline constexpr auto MemberCacheTTL = std::chrono::microseconds{5};
...
inline constexpr auto LeaveDrainWindow = std::chrono::milliseconds{1};
...
inline constexpr auto DeadWindow = std::chrono::milliseconds{500};
...
inline constexpr auto ClockSkewBound = std::chrono::milliseconds{50};
...
inline constexpr auto SweepControlTimeout = std::chrono::milliseconds{2};
...
inline constexpr auto SpinWindow = std::chrono::microseconds{1000};
...
inline constexpr auto RequestWithdrawSettle = std::chrono::microseconds{10};
...
inline constexpr auto ClaimSettle = std::chrono::milliseconds{5};
...
inline constexpr auto LeaseTimeout = std::chrono::milliseconds{200};
// Poll gap while waiting on a held lease, to notice release/handoff promptly.
inline constexpr auto LeasePollGap = std::chrono::milliseconds{2};
// Wall-clock deadline to acquire the lease before failing (covers a full MaxPeers burst).
inline constexpr auto LeaseAcquireDeadline = std::chrono::seconds{5};
